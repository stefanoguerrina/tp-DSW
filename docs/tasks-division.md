...
- Mínimo 1 revisión/aprobación de otro integrante antes de mergear (así todos vemos el código de los demás).
- `develop` → `main` solo en las fechas de finalización de cada fase.

**Dependencias entre fases:**
- La Fase 1 (infra + DB + arquitectura) bloquea a todos, así que se termina primero y recién ahí se arranca en paralelo la Fase 2 en adelante.
- Dentro de cada fase, las 4 tareas son independientes entre sí (por eso están separadas por dev), salvo por entidades compartidas (ver notas al pie de cada fase).
